// Esta funcao roda automaticamente a cada 1 hora
// Sincroniza os dados de hoje de todas as contas Meta Ads
...
exports.handler = async (event) => {
  try {
    if (!META_TOKEN) {
      console.log('META_ACCESS_TOKEN nao configurado')
      return { statusCode: 200, body: 'TOKEN_MISSING' }
    }

    var today = new Date().toISOString().split('T')[0]
    var timeRange = JSON.stringify({ since: today, until: today })
    ...
    if (!Array.isArray(accounts) || accounts.length === 0) {
      ...
      return { statusCode: 200, body: 'NO_ACCOUNTS' }
    }
      ...
      try {
        ...
        for (var ri = 0; ri < rows.length; ri++) {
          ...
          await sbPost('meta_ads_daily', {
            ...
            spend: parseFloat(row.spend || 0) * fxRate,
        ...
        console.log('Sincronizado: ' + account.nome + ' - ' + rows.length + ' registros')
      } catch(e) {
        console.error('Erro ' + account.nome + ': ' + e.message)
      }
    }

    console.log('Cron finalizado. Total: ' + totalSynced + ' registros. USD/BRL: ' + usdToBrl)
    return { statusCode: 200, body: JSON.stringify({ synced: totalSynced, usdToBrl: usdToBrl }) }
